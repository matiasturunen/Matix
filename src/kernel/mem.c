...
/*
 * Implemet kmalloc as a linked list of allocaed segments.
 * segments should be 4 byte aligned
 * Use best fit algorithm to find allocation
 */
...
void * kmalloc(uint32_t bytes) {
	...
	// Add the header to the number of bytes we need and make the size 16 byete aligned
	...
	// If the best difference we found is large, split this segment into two
	// Since our segment headers are quite big, the criterion for splitting is that
	// when split, the segment not being requested should be twice the header size
